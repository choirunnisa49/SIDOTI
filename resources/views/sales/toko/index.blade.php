...
<div class="table-data card">
    <div class="heading-table">
        <div class="row">
            <div class="col-sm-6">
                <h5>List Toko</h5>
                <button type="button" class="btn btn-table" data-bs-toggle="modal" data-bs-target="#form-popup">Tambahkan Toko</button>
            </div>
            <div class="col-sm-6 heading-button">
                <a type="button" class="btn btn-table-phone" data-bs-toggle="modal" data-bs-target="#form-popup">Tambahkan Toko</a>
                <form action="#" class="search">
                    <span><i class="fa fa-search"></i></span>
                    <input type="text" class="search" id="search" onkeyup="doSearch()" placeholder="Cari nama toko...">
                </form>
                    ...
                    <div class="modal-dialog">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title" id="form-popup-label">Tambah Toko</h5>
                                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                            </div>
                            <div class="modal-body">
                                <form action="{{ route('sales.toko.buat') }}" method="POST" enctype="multipart/form-data">
                                    @csrf
                                    <div class="mb-3">
                                        <label for="toko" class="col-form-label">Nama Toko</label>
                                        <input type="text" class="form-control" name="toko" id="toko" placeholder="contoh: Toko Berkah" required autocomplete="off">
                                    </div>
                                    <div class="mb-3">
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <input type="text" class="form-control" name="kecamatan" id="kecamatan" placeholder="contoh: Depok" required autocomplete="off">
                                    </div>
                                    <div class="mb-3">
                                        <label for="kelurahan" class="col-form-label">Kelurahan</label>
                                        <input type="text" class="form-control" name="kelurahan" id="kelurahan" placeholder="contoh: Condongcatur" required autocomplete="off">
                                    </div>
                                    <div class="mb-3">
                                        <label for="alamat" class="col-form-label">Alamat Pabrik</label>
                                        <textarea name="alamat" class="form-control" id="alamat" placeholder="contoh: Jl. Contoh No 00, Rt 00, Rw 00" required autocomplete="off"></textarea>
                                    </div>
                                    <div class="mb-3">
        ...
        <table class="table table-striped table-bordered zero-configuration" id="dataTable">
            <thead>
                <tr>
                    ...
                    <th scope="col">Sedang dijual</th>
                    <th scope="col">Produk yang Laku</th>
                    <th scope="col">Produk yang dikembalikan</th>
                ...
                @endphp
                 @foreach ($toko as $item)
                    <tr>
                        ...
                        <td>{{ $item->provinsi }}</td>
                        <td>{{ $item->kota }}</td>
                        <td>{{ $item->alamat }}, {{ $item->kecamatan }}, {{ $item->kelurahan }}</td>
                        <td>{{ $item->proses_menjual }}</td>
                        <td>{{ $item->terjual }}</td>
                        <td>{{ $item->dikembalikan }}</td>
                        <td>{{ $item->dibuat_oleh }}</td>
                        <td>
                            <a href="{{ url('/sales/toko/lokasi/' . $item->kode_toko) }}" title="Lihat Toko">
                                <button class="btn btn-primary btn-sm">
                                    <i class="bx bx-map" aria-hidden="true"></i>
                                </button>
                            </a>
                            <a href="{{ url('/sales/toko/' .$item->kode_toko. '/edit') }}" title="Ubah Detail Toko">
                                <button class="btn btn-warning btn-sm">
                                    <i class="bx bx-pencil" aria-hidden="true"></i>
                                </button>
                            </a>
                            <a href="{{ url('/sales/toko/hapus/' . $item->kode_toko) }}" class="btn btn-danger btn-sm delete-confirm"><i class="bx bx-trash" aria-hidden="true"></i></a>
                        </td>
                    </tr>
                @endforeach
            </tbody>
...
@section('javascript')
    ...
    <script>
        ...
        swal({
            title: 'Apakah anda yakin menghapus data toko ini?',
        ...
        .then(function(value) {
            if (value.isConfirmed) {
                ...
                swal("Dibatalkan", "Data toko anda tidak jadi dihapus.", "error");
            }
        });
    });
    </script>
